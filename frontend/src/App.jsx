...
function App() {
  ...
  return <div className="min-h-screen bg-[#09090b] text-white">
    ...
    <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8"><div className="mb-8"><p className="mb-2 text-sm font-medium text-indigo-400">EIGI AI HACKATHON · TRACK 02</p><h2 className="text-3xl font-bold tracking-tight sm:text-4xl">Give Navigator a goal.<br /><span className="text-zinc-500">It operates the browser.</span></h2></div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2"><Panel title="Agent Execution Log" subtitle="Observe → Decide → Act → Verify → Recover"><div className="space-y-4">{events.map((event, index) => <EventItem key={`${event.timestamp || "initial"}-${index}`} event={event} />)}</div></Panel><section className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/70"><div className="flex items-center justify-between border-b border-zinc-800 px-6 py-5"><div><h3 className="font-semibold">Browser Session</h3><p className="mt-1 text-xs text-zinc-500">The live browser opens separately from this dashboard.</p></div><span className="rounded-full bg-green-500/10 px-3 py-1 text-xs text-green-400">Airbnb</span></div><div className="flex aspect-video items-center justify-center bg-zinc-950"><div className="text-center"><div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-zinc-900 text-2xl">🏠</div><p className="font-medium text-zinc-300">{running ? "Browser agent is active" : "Browser session ready"}</p></div></div></section></div>
      ...
      <section className="mt-6 rounded-2xl border border-yellow-900/50 bg-yellow-950/20 p-5"><div className="flex gap-4"><div className="text-xl">🛡️</div><div><p className="font-semibold text-yellow-400">Safety Guard Active</p><p className="mt-1 text-sm leading-6 text-zinc-400">Navigator stops before reservation, payment, final booking confirmation, or any other irreversible booking action.</p></div></div></section>
    </main>
  </div>;
}
...
function Brain({ state, status, completedStates }) { return <div className="space-y-2">{BRAIN_STATES.map((item, index) => { const complete = completedStates.includes(item); const active = item === state && status !== "completed"; const failed = status === "failed" && active; return <div key={item} className="flex items-center gap-3"><span className={`flex h-6 w-6 items-center justify-center rounded-full text-xs ${complete ? "bg-green-500/20 text-green-400" : active ? failed ? "bg-red-500/20 text-red-300" : "bg-indigo-500/20 text-indigo-300" : "bg-zinc-800 text-zinc-600"}`}>{complete ? "✓" : failed ? "✕" : active ? "●" : "○"}</span><span className={`text-sm font-medium ${active ? "text-indigo-200" : complete ? "text-zinc-300" : "text-zinc-600"}`}>{item}</span>{index < BRAIN_STATES.length - 1 && <span className="ml-1 text-zinc-700">↓</span>}</div>; })}</div>; }
function PlanItem({ step, index, running, completed }) { const active = running && index === 1; const done = completed || (running && index === 0); return <div className="flex items-center gap-3"><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-xs ${done ? "bg-green-500/20 text-green-400" : active ? "bg-indigo-500/20 text-indigo-300" : "bg-zinc-800 text-zinc-500"}`}>{done ? "✓" : active ? "→" : "○"}</span><span className={active ? "text-sm text-indigo-200" : done ? "text-sm text-zinc-300" : "text-sm text-zinc-500"}>{step.label}</span></div>; }
