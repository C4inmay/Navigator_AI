...
function App() {
  ...
  const [browserPreviewStatus, setBrowserPreviewStatus] = useState("idle");
  ...
  useEffect(() => {
    ...
    setBrowserPreviewStatus("connecting");
    socket.onopen = () => setBrowserPreviewStatus("connected");
    socket.onmessage = (message) => {
      try {
        const event = JSON.parse(message.data);
        if (event.type === "browser_screenshot" && typeof event.image === "string") {
          setLiveScreenshot(event.image);
          setBrowserPreviewStatus("available");
        } else if (event.type === "browser_preview_unavailable") {
          setBrowserPreviewStatus("unavailable");
        }
      } catch {
        setBrowserPreviewStatus("unavailable");
      }
    };
    socket.onerror = () => setBrowserPreviewStatus((status) => status === "available" ? status : "unavailable");
    return () => socket.close();
  }, [runId]);
  ...
  return <div className="min-h-screen bg-[#09090b] text-white">
    ...
    <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8"><div className="mb-8"><p className="mb-2 text-sm font-medium text-indigo-400">EIGI AI HACKATHON · TRACK 02</p><h2 className="text-3xl font-bold tracking-tight sm:text-4xl">Give Navigator a goal.<br /><span className="text-zinc-500">It operates the browser.</span></h2></div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2"><Panel title="Agent Execution Log" subtitle="Observe, decide, act, verify, recover"><div className="space-y-4">{events.map((event, index) => <EventItem key={`${event.timestamp || "initial"}-${index}`} event={event} />)}</div></Panel><section className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/70"><div className="flex items-center justify-between border-b border-zinc-800 px-6 py-5"><div><h3 className="font-semibold">Live Browser</h3><p className="mt-1 text-xs text-zinc-500">Actual Browser Use Chromium session</p></div><span className={`flex items-center gap-2 rounded-full px-3 py-1 text-xs ${liveScreenshot ? "bg-green-500/10 text-green-400" : "bg-zinc-800 text-zinc-400"}`}><span className={`h-1.5 w-1.5 rounded-full ${running && liveScreenshot ? "animate-pulse bg-green-400" : "bg-current"}`} />{liveScreenshot ? "Live" : "Airbnb"}</span></div><div className="flex aspect-video items-center justify-center bg-zinc-950">{liveScreenshot ? <img src={liveScreenshot} alt="Live Chromium browser preview" className="h-full w-full object-contain" /> : <div className="px-6 text-center"><p className="font-medium text-zinc-300">{browserPreviewStatus === "unavailable" ? "Browser preview temporarily unavailable" : running || browserPreviewStatus === "connecting" || browserPreviewStatus === "connected" ? "Preparing live browser preview" : "Browser is idle"}</p><p className="mt-2 text-xs text-zinc-500">{running ? "Navigator is controlling the headless browser." : "Start a task to view the Browser Use session."}</p></div>}</div></section></div>
...
function EventItem({ event, timestamp }) { const safeEvent = event && typeof event === "object" ? event : { message: event }; const recovery = safeEvent.type === "recovery"; const error = safeEvent.type === "error"; const safety = safeEvent.type === "safety"; const time = safeEvent.timestamp ? new Date(safeEvent.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : ""; return <div className="flex items-start gap-3">{timestamp && <span className="w-16 pt-0.5 text-xs text-zinc-600">{time}</span>}<span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${error ? "bg-red-400" : recovery || safety ? "bg-yellow-400" : "bg-green-400"}`} /><span className={`text-sm ${error ? "text-red-300" : safety ? "text-yellow-300" : "text-zinc-300"}`}>{displayValue(safeEvent.message)}</span></div>; }
...
function HistoryCard({ run, onSelect, onDelete }) { const completed = run.status === "completed"; return <article className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-5 sm:p-6"><div className="flex flex-col justify-between gap-4 sm:flex-row"><div><p className={`text-sm font-semibold ${completed ? "text-green-400" : "text-red-300"}`}>{completed ? "Completed" : displayValue(run.status || "Failed")}</p><h3 className="mt-2 max-w-2xl text-base font-medium text-zinc-100">{displayValue(run.task || "Task not available")}</h3><p className="mt-2 text-sm text-zinc-500">{formatDate(run.started_at)} | {formatDuration(run.duration_seconds)} | {pluralize(run.recovery_count, "recovery")}</p></div><div className="flex shrink-0 items-start gap-2"><button onClick={() => onSelect(run.run_id)} className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-medium hover:bg-indigo-500">View Run</button><button onClick={() => onDelete(run.run_id)} className="rounded-lg border border-zinc-700 px-3 py-2 text-sm text-zinc-400 hover:border-red-800 hover:text-red-300">Delete</button></div></div></article>; }

function RunDetail({ run = {}, onBack }) { const safety = run.safety && typeof run.safety === "object" ? run.safety : {}; const constraints = run.constraints && typeof run.constraints === "object" && !Array.isArray(run.constraints) ? run.constraints : {}; const plan = Array.isArray(run.plan) ? run.plan : []; const events = Array.isArray(run.events) ? run.events : []; const finalResult = run.final_result && typeof run.final_result === "object" ? (run.final_result.summary ?? run.final_result) : run.final_result; return <div className="min-h-screen bg-[#09090b] text-white"><header className="border-b border-zinc-800/80 bg-zinc-950/80"><div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-5 sm:px-8"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-600 font-bold">N</div><h1 className="text-2xl font-bold">Navigator<span className="text-indigo-400">AI</span></h1></div><button onClick={onBack} className="rounded-lg border border-zinc-700 px-3 py-2 text-sm text-zinc-300 hover:border-indigo-500">Back to Execution History</button></div></header><main className="mx-auto max-w-5xl px-5 py-8 sm:px-8"><p className="text-sm font-medium text-indigo-400">NAVIGATOR RUN</p><h2 className="mt-2 text-3xl font-bold">Execution Report</h2><div className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-4"><DetailMetric label="Status" value={run.status === "completed" ? "Completed" : displayValue(run.status || "Failed")} good={run.status === "completed"} /><DetailMetric label="Duration" value={formatDuration(run.duration_seconds)} /><DetailMetric label="Recoveries" value={displayValue(run.recovery_count ?? 0)} /><DetailMetric label="Human interventions" value={displayValue(run.human_interventions ?? 0)} /></div><DetailSection title="User Task"><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-300">{displayValue(run.task)}</p></DetailSection><DetailSection title="Agent Plan"><div className="space-y-2">{plan.length ? plan.map((item, index) => <HistoryPlanItem key={`${planLabel(item)}-${index}`} item={item} />) : <p className="text-sm text-zinc-500">Not available</p>}</div></DetailSection><DetailSection title="Constraints"><div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{Object.keys(constraints).length ? Object.entries(constraints).map(([key, value]) => <div key={key} className="rounded-lg bg-zinc-950/50 p-3"><p className="text-xs uppercase tracking-wider text-zinc-500">{key}</p><p className="mt-1 whitespace-pre-wrap text-sm text-zinc-200">{displayValue(value)}</p></div>) : <p className="text-sm text-zinc-500">Not available</p>}</div></DetailSection><DetailSection title="Execution Timeline"><div className="space-y-4">{events.length ? events.map((event, index) => <EventItem key={`${event?.timestamp || "event"}-${index}`} event={event} timestamp />) : <p className="text-sm text-zinc-500">No event history is available.</p>}</div></DetailSection><DetailSection title="Recovery"><p className="text-sm text-zinc-300">{pluralize(run.recovery_count, "recovery event")}</p></DetailSection><DetailSection title="Final Result"><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-300">{displayValue(finalResult)}</p></DetailSection><DetailSection title="Safety"><div className="space-y-2 text-sm text-zinc-300"><p>Reservation attempted: {safety.reservation_attempted ? "YES" : "NO"}</p><p>Payment attempted: {safety.payment_attempted ? "YES" : "NO"}</p><p className={safety.safety_stop ? "text-yellow-300" : "text-zinc-500"}>Safety stop: {safety.safety_stop ? "YES" : "Not available"}</p></div></DetailSection></main></div>; }
...
function HistoryPlanItem({ item }) { const state = typeof item === "object" && item !== null ? item.state : null; const tone = state === "completed" ? "bg-green-400" : state === "active" ? "bg-indigo-400" : "bg-zinc-700"; return <div className="flex items-center gap-3"><span className={`h-2.5 w-2.5 shrink-0 rounded-full ${tone}`} /><p className="text-sm text-zinc-300">{planLabel(item)}</p></div>; }
function planLabel(item) { return typeof item === "object" && item !== null && "label" in item ? displayValue(item.label) : displayValue(item); }
function displayValue(value) { if (value === null || value === undefined || value === "") return "Not available"; if (["string", "number", "boolean"].includes(typeof value)) return String(value); try { return JSON.stringify(value, null, 2); } catch { return "Not available"; } }
